...
class TOM90ExactRenderer:
    ...
    def _load_page_data(self, page_num: int) -> Dict[str, Any]:
                ...
                for block in content.split("-" * 40):
                    ...
                    for line in lines:
                        if line.startswith("Text:"):
                            ...
                            text_data["bbox"] = ast.literal_eval(line.split("Position:", 1)[1].strip())
                        elif line.startswith("Schriftgröße:"):
                            text_data["size"] = float(line.split("Schriftgröße:", 1)[1].strip())
        ...
        # Bilder laden
        img_path = os.path.join(self.input_dir, f"seite_{page_num}_bilder_positionen.txt")
                ...
                for line in f:
                    ...
                    if match:
                        ...
                        for ext in ("png", "jpg", "jpeg", "tif", "tiff"):
                            candidate = os.path.join(self.input_dir, f"seite_{page_num}_bild_{idx}.{ext}")
    ...
    def _substitute_dynamic_content(self, text: str, page_num: int) -> str:
        ...
        if "Sehr geehrte" in text or "Kunde" in text:
            customer_name = self.project_data.get('customer_name', 'Sehr geehrte Kundin, sehr geehrter Kunde')
            return customer_name
        
        # Kundenadresse ersetzen
        if any(addr_part in text for addr_part in ["Musterstraße", "12345", "Musterstadt"]):
    ...
    def _get_value_for_key(self, key: str) -> str:
        ...
        if key == 'customer_name':
            ...
            address = self.project_data.get('customer_address', {})
            return address.get('street', 'Musterstraße 1') if isinstance(address, dict) else str(address)
        elif key == 'customer_address_city':
            address = self.project_data.get('customer_address', {})
            return address.get('city', '12345 Musterstadt') if isinstance(address, dict) else ''
        ...
        # FIRMENDATEN
            ...
            company_address = self.company_info.get('address', {})
            return company_address.get('street', 'Maximilianstraße 35') if isinstance(company_address, dict) else ''
        elif key == 'company_address_city':
            company_address = self.company_info.get('address', {})
            return company_address.get('city', '80539 München') if isinstance(company_address, dict) else ''
        ...
        # FINANZIELLE ERGEBNISSE
            ...
            savings = self.analysis_results.get('total_savings_without_storage_eur', 29150)
            return f"{savings:,.0f}".replace(",", ".")
        ...
        # TECHNISCHE DATEN
            ...
            return str(self.project_data.get('battery_details', {}).get('capacity_kwh', 6.1))
            ...
            return str(self.project_data.get('pv_details', {}).get('module_quantity', 21))
        elif key == 'roof_angle':
    ...
    def _adjust_text_position(self, text: str, x: float, y: float, page_num: int) -> Tuple[float, float]:
        ...
        if page_num == 1:
            if any(keyword in text for keyword in ["IHRE PERSÖNLICHE PV-ANLAGE, IM ÜBERBLICK"]):
                return x, y + 13.35
        ...
        ersparnis_keywords = [
            ...
            "inkl. Photovoltaikanlage", "Fördergelder"
        ]
        ...
        tech_keywords = [
            "Heizung", "Warmwasser", "Verbrauch", "Dachneigung",
            "Solaranlage", "Batterie", "Jahresertrag",
            "Wärmepumpe", "Elektrischer Boiler",
            "6.000 kWh/Jahr", "30°", "8,4 kWp", "6,1 kWh", "8.251,92 kWh/Jahr"
        ]
    ...
    def _create_page_from_data(self, doc: fitz.Document, page_num: int) -> None:
            ...
            if img_data['file'] and os.path.exists(img_data['file']):
                try:
                    ...
                    if page_num == 1 and self.company_logo_base64:
                        ...
                        page.insert_image(fitz.Rect(*img_data['bbox']), stream=logo_data)
                    else:
                        page.insert_image(fitz.Rect(*img_data['bbox']), filename=img_data['file'])
                except Exception as e:
        ...
        for text_data in page_data['texts']:
            try:
                ...
                display_text = self._substitute_dynamic_content(text_data['text'], page_num)
                
                # Position justieren für spezifische Texte
                adjusted_x, adjusted_y = self._adjust_text_position(display_text, x0, y0, page_num)
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"Fehler beim Text einfügen '{display_text}': {text_error}")
                        continue
                
            except Exception as e:
                print(f"Fehler beim Einfügen von Text '{text_data.get('text', '')}': {e}")
    ...
    def _create_page1_cover(self, doc: fitz.Document) -> None:
        ...
        page.draw_line(
            (297.50, 365.69), (554.53, 365.69),
            color=(0,0,0), width=0.5
        )
        page.draw_line(
            (297.50, 394.17), (554.53, 394.17),
            color=(0,0,0), width=0.5
        )
        page.draw_line(
            (418.90, 364.94), (418.90, 422.64),
            color=(0,0,0), width=0.5
        )
        ...
        page.draw_rect(chart_rect, color=self._rgb_from_int(self.tom90_blue), width=1)
        page.insert_text(
            (60, 640),
            "Ersparnis-Diagramm\n(wird hier eingefügt)",
            fontname="helv",
    ...
    def _create_page2_analysis(self, doc: fitz.Document) -> None:
        ...
        page.insert_text((120, 300), "HAUS", fontname="helv-bold", fontsize=10, color=self._rgb_from_int(self.tom90_blue))
        
        # PV-Anlage  
        pv_rect = fitz.Rect(100, 150, 200, 200)
        page.draw_rect(pv_rect, color=self._rgb_from_int(self.tom90_orange), width=2)
        page.insert_text((110, 175), "PV-ANLAGE", fontname="helv-bold", fontsize=10, color=self._rgb_from_int(self.tom90_blue))
        
        # Batterie
        battery_rect = fitz.Rect(250, 250, 350, 350)
        page.draw_rect(battery_rect, color=self._rgb_from_int(self.tom90_blue), width=2)
        page.insert_text((270, 300), "BATTERIE", fontname="helv-bold", fontsize=10, color=self._rgb_from_int(self.tom90_blue))
        
        # Stromnetz
        grid_rect = fitz.Rect(100, 400, 200, 450)
        page.draw_rect(grid_rect, color=self._rgb_from_int(self.tom90_blue), width=2)
        page.insert_text((110, 425), "STROMNETZ", fontname="helv-bold", fontsize=10, color=self._rgb_from_int(self.tom90_blue))
        ...
        for start, end in arrows:
            page.draw_line(start, end, color=self._rgb_from_int(self.tom90_blue), width=2)
            # Pfeilspitze (vereinfacht)
            page.draw_line((end[0]-5, end[1]-5), end, color=self._rgb_from_int(self.tom90_blue), width=2)
            page.draw_line((end[0]+5, end[1]-5), end, color=self._rgb_from_int(self.tom90_blue), width=2)
    ...
    def _create_page3_chart(self, doc: fitz.Document) -> None:
        ...
        page.draw_rect(with_rect, color=self._rgb_from_int(self.tom90_orange), fill=True)
        ...
        page.draw_rect(without_rect, color=self._rgb_from_int(self.tom90_blue), fill=True)
        ...
        # Chart-Achsen
        page.draw_line((80, 180), (80, 330), color=self._rgb_from_int(self.tom90_blue), width=1)  # Y-Achse
        page.draw_line((80, 330), (350, 330), color=self._rgb_from_int(self.tom90_blue), width=1)  # X-Achse
    ...
    def _create_page4_components(self, doc: fitz.Document) -> None:
        ...
        if self.project_data.get('pv_details', {}).get('include_storage'):
            battery_capacity = self.project_data.get('project_details', {}).get('battery_capacity_kwh', 6.1)
    ...
    def _create_page5_facts(self, doc: fitz.Document) -> None:
        ...
        earth_radius = 50
        page.draw_circle(earth_center, earth_radius, color=self._rgb_from_int(self.tom90_blue), width=2)
        page.insert_text(
