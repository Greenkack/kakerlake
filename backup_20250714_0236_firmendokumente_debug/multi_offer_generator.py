...
class MultiCompanyOfferGenerator:
    ...
    def render_offer_configuration(self):
        ...
        if "pdf_options" not in settings:
            settings["pdf_options"] = {
                ...
                "include_company_documents": True,  # NEU: Firmendokumente standardmäßig aktiviert
        ...
        with pdf_cols[0]:
            ...
            # Neue Option für Firmendokumente-Auswahl
            pdf_options["include_company_documents"] = st.checkbox(
                "Firmendokumente einbinden",
                value=pdf_options.get("include_company_documents", True),
                help="Firmendokumente automatisch in PDFs einbinden"
            )
            
            if pdf_options["include_company_documents"]:
                # Firmen-Dokumente-Modi
                doc_mode = st.selectbox(
                    "Dokument-Modus:",
                    ["Alle verfügbaren", "Nur Standard-Typen", "Erste 3 Dokumente"],
                    index=pdf_options.get("company_docs_mode_index", 0),
                    help="Wie sollen Firmendokumente ausgewählt werden?"
                )
                pdf_options["company_docs_mode"] = doc_mode
                pdf_options["company_docs_mode_index"] = ["Alle verfügbaren", "Nur Standard-Typen", "Erste 3 Dokumente"].index(doc_mode)
                
                if doc_mode == "Nur Standard-Typen":
                    standard_doc_types = st.multiselect(
                        "Standard-Dokumenttypen:",
                        ["Katalog", "Zertifikat", "Datenblatt", "Garantie", "Anleitung"],
                        default=pdf_options.get("standard_doc_types", ["Katalog", "Datenblatt"]),
                        help="Diese Dokument-Typen werden für alle Firmen eingefügt"
                    )
                    pdf_options["standard_doc_types"] = standard_doc_types
    ...
    def get_company_documents_for_pdf(self, company: Dict, pdf_options: Dict) -> List[int]:
        """
        Ermittelt die Firmendokument-IDs, die für eine spezifische Firma 
        in das PDF eingefügt werden sollen
        """
        if not pdf_options.get("include_company_documents", True):
            return []
        ...
        try:
            # Lade alle verfügbaren Firmendokumente
            from database import list_company_documents
            if not callable(list_company_documents):
                logging.warning("list_company_documents Funktion nicht verfügbar")
                return []
            
            all_docs = list_company_documents(company_id, None)  # Alle Dokument-Typen
            if not all_docs:
                logging.info(f"Keine Firmendokumente für Firma {company.get('name', company_id)} gefunden")
                return []
            
            # Dokument-Auswahl basierend auf Modus
            doc_mode = pdf_options.get("company_docs_mode", "Alle verfügbaren")
            
            if doc_mode == "Alle verfügbaren":
                selected_docs = all_docs
                
            elif doc_mode == "Nur Standard-Typen":
                standard_types = pdf_options.get("standard_doc_types", ["Katalog", "Datenblatt"])
                selected_docs = [doc for doc in all_docs if doc.get("document_type") in standard_types]
                
            elif doc_mode == "Erste 3 Dokumente":
                selected_docs = all_docs[:3]  # Erste 3 Dokumente
                
            else:
                selected_docs = all_docs  # Fallback
            
            # IDs extrahieren
            doc_ids = [doc.get("id") for doc in selected_docs if doc.get("id")]
            
            logging.info(f"Firma {company.get('name', company_id)}: {len(doc_ids)} Dokumente ausgewählt ({doc_mode})")
            if doc_ids:
                doc_names = [doc.get("display_name", "Unbekannt") for doc in selected_docs if doc.get("id")]
                logging.info(f"  Dokumente: {', '.join(doc_names[:3])}{'...' if len(doc_names) > 3 else ''}")
            
            return doc_ids
            
        except Exception as e:
            logging.error(f"Fehler beim Laden der Firmendokumente für Firma {company_id}: {e}")
            return []
        ...
        try:
            ...
            if callable(generate_offer_pdf):
                ...
                if not pdf_options.get("include_visualizations", True):
                  ...
                  # NEUE FEATURE: Firmendokumente für diese spezifische Firma laden
                company_document_ids = self.get_company_documents_for_pdf(company, pdf_options)
                ...
                pdf_content = generate_offer_pdf(
                    ...
                    inclusion_options={
                        ...
                        "company_document_ids_to_include": company_document_ids,  # ECHTE FIRMENDOKUMENT-IDs!
